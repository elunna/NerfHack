...
option); also for ``\texttt{\#vanquished}'' and ``\texttt{\#exiled}'' commands
...
doing that for either \texttt{\#exiled} or \texttt{\#vanquished} changes the order for both.
\\
%.lp ""
...
for \texttt{\#exiled} and alphabetical will be used instead.
The menu omits those two choices when used for \texttt{\#exiled}.
\\
%.lp ""
Autocompletes.
...
The \texttt{\#exiled} command shares this sorting order.
\\
%.lp ""
...
set in your run-time configurtion file or \texttt{NERFHACKOPTIONS} environment
variable.
...
enabled by setting the correspondingly named option in \texttt{NERFHACKOPTIONS}
...
file, or in the ``\texttt{NERFHACKOPTIONS}'' environment variable.
Some versions of \textit{NerfHack} also have front-end programs that allow
...
text editor.
After running \textit{NerfHack} for the first time, you should find a default
template for ths configuration file named \mbox{``.nerfhackrc.template''} in
\mbox{{``\%USERPROFILE\%\textbackslash NerfHack\textbackslash''}}.
If you have not created the configuration file, \textit{NerfHack} will create
the configuration file for you using the default template file.\\

%.lp ""
On MS-DOS it is \mbox{``defaults.nh''} in the same folder as
\mbox{\textit{nerfhack.exe}}.\\
...
Default location of files \textit{NerfHack} needs. On Windows HACKDIR
defaults to the location of the \textit{NerfHack.exe} or \textit{NerfHackw.exe} file
...
%.hn 2
\subsection*{Using the \texttt{NERFHACKOPTIONS} environment variable}
...
\texttt{g} --- list monster species that have been exiled;\\
...
\texttt{exiled} to \textit{not disclose} and \textit{not prompt},
...
and also for the \texttt{\#exiled} command.
Persistent.
\\
%.lp ""
The possible values are:
...
height needed.
(If NerfHack has been built using a version of \texttt{Qt}

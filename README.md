...
On Windows, from a **Developer Command Prompt for VS** (the same environment used to build NerfHack itself - the script will tell you if you're not in one):

```
sys\windows\nerfhack-gen-tilesets.bat
```

Both scripts do the same thing: build `tile2bmp` from current game data, generate the base 16x16 sheet, and upscale it to 32x32, 64x64, and 128x128, writing everything to `tilesets/` (gitignored - these are large generated files, not something to commit). Pass specific factors to generate only those sizes, e.g. `sys/unix/nerfhack-gen-tilesets.sh 2 4` (or `sys\windows\nerfhack-gen-tilesets.bat 2 4` on Windows) for just 32x32 and 64x64. The upscaler itself (`sys/unix/nerfhack-scale-bmp.py`, shared by both) only needs Python 3's standard library and can be reused standalone on any 8-bit indexed BMP.

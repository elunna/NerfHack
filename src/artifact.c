...
/* adtyp_str(), artifact_info(), and get_faux_artifact_obj() now live in
 * olookup.c */
...
invoke_healing(struct obj *obj)
{
    ...
    if (healamt || Sick || Slimed || HWithering || Rabid || Blinded > creamed)
        You_feel("better.");
...
staticfn int
invoke_uncurse(struct obj *obj UNUSED)
{
...
staticfn int
invoke_lightning(struct obj *obj UNUSED)
{
    struct obj pseudo;
    pseudo = cg.zeroobj;
...
arti_invoke(struct obj *obj)
{
    ...
    if (oart->inv_prop > LAST_PROP) {
        ...
        case SUMMONING: res = invoke_summoning(obj); break;
        case UNCURSE_INVK: res = invoke_uncurse(obj); break;
        case LIGHTNING_BOLT: res = invoke_lightning(obj); break;
        default:

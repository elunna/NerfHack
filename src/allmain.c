...
welcome(boolean new_game) /* false => restoring an old game */
{
    ...
    if (Hallucination)
        pline("HACKEM MUCHE is filmed in front of an undead studio audience.");
    ...
    *buf = '\0';
    ...
    pline(new_game ? "%s %s, welcome to HACKEM MUCHE!  You are a%s."
                   : "%s %s, the%s, welcome back to HACKEM MUCHE!",

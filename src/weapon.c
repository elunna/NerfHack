...
#define is_odd_material(obj, mat) \
    ((obj)->material == (mat) && !(objects[(obj)->otyp].oc_material == (mat)))
        if (is_odd_material(otmp, GLASS)) {
            ...
            || is_odd_material(otmp, COLDSTEEL)) {
...
#undef is_odd_material
    ...
    /* Rage oprop - weapons of rage get a generous double damge bonus */
    if (otmp->oprops & ITEM_RAGE)
        tmp *= 2;
    ...
    if (Is_weapon || otmp->oclass == GEM_CLASS || otmp->oclass == BALL_CLASS
        ...
        if (otmp->material == BONE && ptr && noncorporeal(ptr)) {
    ...
    boolean minimal) /* print a shorter message leaving out obj details */
{
    ...
    if (mat == SILVER || mat == COLDSTEEL) { /* more dramatic effects than other materials */
                 ...
                 struct obj *best)
{
    ...
    for (otmp = start; otmp; otmp = otmp->nobj) {
        ...
        /* don't consider the off-hand weapon for the primary slot - it
           would end up double-wielded as both mw and mw2 at once */
        if (otmp == MON_WEP2(mtmp))
            continue;
    ...
    struct obj *best)
{
    ...
    for (otmp = start; otmp; otmp = otmp->nobj) {
        ...
        if (otmp->oclass == WEAPON_CLASS && otmp->oartifact
            && otmp != MON_WEP2(mtmp) /* don't double-wield the off-hand */
...
boolean
can_dual_wield(struct monst *mon)
{
    ...
    if ((mwep->oclass == WEAPON_CLASS || is_weptool(mwep))
...
/* Select a secondary hand-to-hand weapon for dual-wielding. Simpler
   than select_hwep(): skips the primary weapon, requires a one-handed
   weapon, and scores by raw damage/enchantment/artifact rather than
   the full hwep[] type-priority table. */
struct obj *
select_offhand_hwep(struct monst *mon)
{
    ...
    for (otmp = mon->minvent; otmp; otmp = otmp->nobj) {
        ...
        if (otmp == MON_WEP(mon))
            continue;
        /* skip worn/wielded items (but allow the current secondary,
           so it can be re-scored against the rest of the inventory) */
        ...
        if (objects[otmp->otyp].oc_bimanual)
            continue;
        /* skip launchers - off-hand is for melee */
        if (is_launcher(otmp))
            continue;
        if (mon_hates_material(mon, otmp->material))
            continue;

        /* score: base damage + enchantment + artifact bonus */
...
int
mon_wield_offhand(struct monst *mon)
{
    struct obj *obj;
    ...
    if (artifact_light(obj) && !obj->lamplit) {
        ...
        if (canseemon(mon))
            ...
            pline("Light begins shining %s.",
                  (dist2(mon->mx, mon->my, u.ux, u.uy) <= 5 * 5)
                      ? "nearby"
                      : "in the distance");
    }
...
possibly_unwield(struct monst *mon, boolean polyspot)
{
 ...
 check_mw2:
    /* also validate the secondary weapon, if any */
    if ((mw_tmp = MON_WEP2(mon)) != 0) {
        ...
        if (!obj) {
            ...
            /* can no longer dual-wield (form change, shield, etc.) */
...
void
setmnotwielded2(struct monst *mon, struct obj *obj)
{
    if (!obj)
        return;
    if (artifact_light(obj) && obj->lamplit) {

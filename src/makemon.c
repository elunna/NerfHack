...
static const struct monster_pairing monster_pairings[] = {
    ...
    { PM_HILL_GIANT,         PM_HILL_GIANT_SHAMAN,-1,      TRUE },
    { PM_STONE_GIANT,        PM_HILL_GIANT_SHAMAN,-1,      TRUE },
    { PM_FIRE_GIANT,         PM_HILL_GIANT_SHAMAN,-1,      TRUE },
    { PM_FROST_GIANT,        PM_HILL_GIANT_SHAMAN,-1,      TRUE },
    { PM_STORM_GIANT,        PM_TITAN,         -1,         TRUE },
...
#undef add_ac
	    ...
	    case 3:
                ...
                /*FALLTHRU*/
            case 4:
                (void) mongets(mtmp, rn2(4) ? WAN_MAGIC_MISSILE : WAN_TELEPORTATION);
	    }
        } else if (ptr->msound == MS_PRIEST
                   || quest_mon_represents_role(ptr, PM_CLERIC)) {
...
mongets(struct monst *mtmp, int otyp)
{
    ...
    if (otmp) {
        if (mtmp->data->mlet == S_DEMON) {
            ...
            if (otmp->alignment)
                set_alignment(otmp, !rn2(3) ? sgn(mtmp->data->maligntyp) + 2 : FA_NONE);
        ...
        if (otmp->alignment && item_vs_mon(otmp, mtmp))
            set_alignment(otmp, sgn(mtmp->data->maligntyp) + 2);

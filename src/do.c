...
staticfn void dotoiletamulet(struct obj *);
staticfn void polymorph_toilet(void);
...
polymorph_sink(void)
{
    ...
    case 4:
     ...
     case 5:
        ...
        if (sinklooted)
            levl[u.ux][u.uy].looted = (S_LPOOPY | S_LTOOL);
        break;
    }
...
staticfn void
polymorph_toilet(void)
{
    uchar sym = S_toilet;
    boolean toiletlooted;

    if (levl[u.ux][u.uy].typ != TOILET)
        return;

    toiletlooted = levl[u.ux][u.uy].looted != 0;

    /* svl.level.flags.ntoilets--; */  // set_levltyp() will update this */
    levl[u.ux][u.uy].flags = 0;
    set_levltyp(u.ux, u.uy, SINK); /* updates level.flags.nsinks */

    if (toiletlooted)
        levl[u.ux][u.uy].looted = (S_LPUDDING | S_LDWASHER | S_LRING);
    /* give message even if blind; we know we're not levitating,
       so can feel the outcome even if we can't directly see it */
    if (levl[u.ux][u.uy].typ != ROOM)
        ...
        case OBJ_CONTAINED: {
            ...
            if (!container) {
                ...
                pline("%s %s out of %s in your pack!", mnam,
                      vtense(mnam, locomotion(mtmp->data, "writhe")),
                      sackname);

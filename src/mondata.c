...
{
    struct permonst *ptr;
    /* Always check player's race data, not their role data! */
    if (mon == &gy.youmonst && !Upolyd) {
        ptr = &mons[gu.urace.mnum];
    } else
        ptr = mon->data;

    if (hates_material(ptr, material))
        return TRUE;
...
hates_material(struct permonst *ptr, int material)
{
    if (material == SILVER) {
        ...
        return (ptr->mlet == S_FUNGUS || is_migo(ptr)
                || dmgtype(ptr, AD_DISE)
                || dmgtype(ptr, AD_DCAY)
                || dmgtype(ptr, AD_PEST));
...
static const short grownups[][2] = {
    ...
    { PM_GIANT_MIMIC, PM_KILLER_MIMIC },
    { PM_BABY_LONG_WORM, PM_LONG_WORM },
    { PM_BABY_PURPLE_WORM, PM_PURPLE_WORM },
    { PM_CARRION_LARVA, PM_CARRION_CRAWLER },

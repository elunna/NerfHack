    ...
    if (Role_if(PM_KNIGHT) && u.ualign.type == A_LAWFUL
        ...
        if (mtmp->mpeaceful && canseemon(mtmp)) {
            ...
            if (rn2(3))
                return;
            switch (rnd(5)) {
            case 1: You("slaughter at will.");
                break;
            case 2: pline("%s approves of your carnage.", u_gname());
                break;
            case 3: pline("Yes... More blood!");
                break;
            case 4: verbalize("Might makes right!");
                break;
            case 5: verbalize("Puny weaklings!");
                break;
            }
            return;
        }
        if (helpless(mtmp) && canseemon(mtmp)) {
            ...
            if (rn2(3))
                return;
            switch (rnd(5)) {
            case 1: verbalize("Flounder before me!");
                break;
            case 2: pline_mon(mtmp, "%s struggles helplessly.", Monnam(mtmp));
                break;
            case 3: You("strike down the weak!");
                break;
            case 4: verbalize("No mercy!");
                break;
            case 5: verbalize("Easy kill!");
                break;
            }
            return;
        }
        if (mtmp->mflee && canseemon(mtmp) && !rn2(3)) {
            adjalign(1);
            switch (rnd(3)) {
            case 1: verbalize("Coward!");
                break;
            case 2: verbalize("Get over here!");
                break;
            case 3: verbalize("Easy target!");
                break;
            }
            return;
        }
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (!hmd->train_weapon_skill || mon == u.ustuck
        ...
        ; /* no special bonuses */
    } else if (hmd->mdat->mlet == S_VAMPIRE && obj->otyp == STAKE
    ...
    if (hmd->hand_to_hand) {
         ...
         * crack_worn_obj() on the weapon here and set obj to null.
...
#undef useup_eggs
    ...
    case ACID_VENOM: /* thrown (or spit) */
        ...
        break;
    default:
        if ((obj->material == VEGGY || obj->material == PAPER) &&
    ...
    struct obj *obj)    /* obj can be NULL but guards are in place below */
{
    if ((hmd->mdat == &mons[PM_BLACK_PUDDING]
        ...
        && ((obj->material == IRON || obj->material == COLDSTEEL
    ...
    if (magr == &gy.youmonst) {
        ...
        if (mattk->aatyp == AT_HUGS && !sticks(pd)) {
            ...
            if (mattk->aatyp == AT_WEAP && otmp) {
                ...
                crack_worn_obj(some_armor(mdef));
                if (crack_worn_obj(MON_WEP(magr))) {
                    otmp = NULL;
                }
...
shield_dmg(struct obj *obj, struct monst *mon)
{
    ...
    if (uarms && P_SKILL(P_SHIELD) >= P_BASIC) {
        ...
        if (mon_hates_material(mon, obj->material)) {
            /* extra damage already applied by dmgval() */
            searmsg(&gy.youmonst, mon, obj, TRUE);
        }
